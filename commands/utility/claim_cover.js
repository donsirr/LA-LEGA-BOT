...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('claim_cover')
        .setDescription('(REFEREE) Claim cover referee spot for a match')
    ...
    async execute(interaction) {
        ...
        // Step 1: Fetch match row with all columns A-K
        let sheetData;
        try {
        ...
        // Step 2: Find the row index and data
        ...
        // Step 3: Check if Cover Referee already claimed
        if (coverRefereeId && coverRefereeId.trim() !== '') {
            return interaction.reply({ content: '❌ Cover Referee spot is already claimed for this match.', ephemeral: true });
        }
        ...
        // Step 5: Update embed with new cover referee mention
        ...
        // Step 7: Update Google Sheets cover referee column (H, index 7)
        const sheetRange = `MATCHES!H${rowIndex + 2}`;
        try {
        ...
        return interaction.reply({ content: `✅ You claimed the cover referee spot for match \`${matchId}\`.`, ephemeral: true });
    },
};
